...
["/cmd/watch.js", import('data:text/javascript;text,let type_renderRes,p_target,srcBy$src,eval2,q_eval2,kebabToCamelCase;self.__p.add(import.meta.__imports=Promise.all([self.__i.get("/render/render.js"),self.__i.get("/config.js"),self.__i.get("/descr.js"),self.__i.get("/eval2.js"),self.__i.get("/str.js")]).then(arr=>{let m;m=arr[0];type_renderRes=m.type_renderRes;m=arr[1];p_target=m.p_target;m=arr[2];srcBy$src=m.srcBy$src;m=arr[3];eval2=m.eval2;q_eval2=m.q_eval2;m=arr[4];kebabToCamelCase=m.kebabToCamelCase;}));export default{render(r){return eval2(r,r.$src,true).then(e=>watch_render(r,r.scope,getName(r),e))},q_render(a,c,d){return q_eval2(a,c,d).then(r=>{const t=getName(a),n=c.length,s=new Array(n);for(let e=0;e<n;e++){if(!d.has(e)){s[e]=watch_render(a,c[e].scope,t,r[e])}}return s})}};function getName(e){const r=e.reqCmd.args[0];if(r!==undefined&&r!==""){return kebabToCamelCase(r)}}function watch_render(e,r,t,n){const s=srcBy$src.get(e.$src).cache;if(!s.current.has(e.str)){s.current.set(e.str,type_watchCur())}const a=s.current.get(e.str);if(e.sync.renderParam.isLinking){s.isInits.add(e.str);a.watch=n;return type_renderRes(true)}if(s.isInits.has(e.str)){if(n===a.watch){return type_renderRes(true)}e.sync.onreadies.add(()=>{a.watch=n})}else{e.sync.onreadies.add(()=>{s.isInits.add(e.str);a.watch=n})}if(t!==undefined){r[p_target][t]=n}return null}function type_watchCur(){return{watch:undefined}}')],
["/api.js", import('data:text/javascript;text,let incCache,render,getCurRender,setDelay,p_target,p_topUrl,preRender,removeChild,dispatchCustomEvent,setLoc,getProxy,oset,del,copyToClipboard,getUrl;self.__p.add(import.meta.__imports=Promise.all([self.__i.get("/cmd/inc.js"),self.__i.get("/render/algo.js"),self.__i.get("/config.js"),self.__i.get("/dom.js"),self.__i.get("/evt.js"),self.__i.get("/loc.js"),self.__i.get("/proxy.js"),self.__i.get("/oset.js"),self.__i.get("/str.js"),self.__i.get("/url.js")]).then(arr=>{let m;m=arr[0];incCache=m.incCache;m=arr[1];render=m.render;getCurRender=m.getCurRender;setDelay=m.setDelay;m=arr[2];p_target=m.p_target;p_topUrl=m.p_topUrl;m=arr[3];preRender=m.preRender;removeChild=m.removeChild;m=arr[4];dispatchCustomEvent=m.dispatchCustomEvent;m=arr[5];setLoc=m.setLoc;m=arr[6];getProxy=m.getProxy;m=arr[7];oset=m.oset;del=m.del;m=arr[8];copyToClipboard=m.copyToClipboard;m=arr[9];getUrl=m.getUrl;}));function begin(){self.mw_incCache=incCache;self.mw_render=render;self.mw_getCurRender=getCurRender;self.mw_setDelay=setDelay;self.mw_p_target=p_target;self.mw_p_topUrl=p_topUrl;self.mw_preRender=preRender;self.mw_removeChild=removeChild;self.mw_setLoc=setLoc;self.mw_getProxy=getProxy;self.mw_oset=oset;self.mw_del=del;self.mw_copyToClipboard=copyToClipboard;self.mw_getUrl=getUrl;self.mw_dispatchCustomEvent=dispatchCustomEvent}if(self.__imports===undefined){begin()}else{self.__imports.then(begin)}')],
...
["/oset.js", import('data:text/javascript;text,let p_target;self.__p.add(import.meta.__imports=Promise.all([self.__i.get("/config.js")]).then(arr=>{let m;m=arr[0];p_target=m.p_target;}));export function oset(t,e,n){const o=t[e];if(typeof o!=="object"||o===null||typeof n!=="object"||n===null){t[e]=n;return true}if(Array.isArray(o)){return osetArray(t,e,n)}if(o instanceof Set){return osetSet(t,e,n)}if(o instanceof Map){return osetMap(t,e,n)}return osetObject(t,e,n)}function osetArray(t,e,n){if(!Array.isArray(n)){t[e]=n;return true}const o=t[e],r=o.length,f=n.length;for(let t=0;t<r&&t<f;t++){oset(o,t,n[t])}const c=r-f;if(c>0){o.splice(f,c);return true}for(let t=r;t<f;t++){o.push(n[t])}return true}function osetObject(t,e,n){if(typeof n!=="object"||n===null){t[e]=n;return true}const o=t[e];for(const r in o){if(!(r in n)){delete o[r]}}for(const r in n){oset(o,r,n[r])}return true}function osetSet(t,e,n){const o=t[e];if(n instanceof Set){for(const r of o){if(!n.has(r)){o.delete(r)}}for(const r of n){o.add(r)}}o.clear();return Reflect.set(t,e,n)}function osetMap(t,e,n){const o=t[e];if(n instanceof Map){for(const[r,f]of o){if(!n.has(r)){o.delete(r)}}for(const[r,f]of n){const c=o.get(r);if(typeof c==="object"){if(Array.isArray(c)){c.splice(0,c.length)}else if(c instanceof Set||c instanceof Map){c.clear()}else if(c!==null){for(const s of c){delete c[s]}}}o.set(r,f)}}o.clear();return Reflect.set(t,e,n)}export function del(t,e){const n=t[e];delete t[e];return n}export function ocopy(t){const e={};for(const n in t){e[n]=t[n]}return e}export function srcSetScope(t,e){const n=t.scopeCache,o=n[p_target],r=e[p_target],f=r!==undefined?r:e;for(const c in f){if(c in o){continue}o[c]=f[c]}return n}')],
