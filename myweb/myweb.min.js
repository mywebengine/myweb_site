 ...
 */
await Promise.all(self.__import__=new Map([["/loading.js", import('data:text/javascript;text,let isFillingName,isFillingDiv,srcBy$src,is$hide;import.meta.__imports__=Promise.all([self.__import__.get("/config.js"),self.__import__.get("/descr.js"),self.__import__.get("/dom.js")]).then(arr=>{let m;m=arr[0];isFillingName=m.isFillingName;isFillingDiv=m.isFillingDiv;m=arr[1];srcBy$src=m.srcBy$src;m=arr[2];is$hide=m.is$hide;});export const loadingCount=new Map;export async function showLoading(n,e,t="",i){const o=getLoadingKey(n),g=loadingCount.get(o);if(await e()){if(g!==undefined){decLoading(n,t,g)}return}const d=g===undefined?createLoading(o):g;if(i===undefined||i===""){i=-1}if(i<0){toggleLoading(n,"",true,d);if(t!==""){toggleLoading(n,t,true,d)}return}toggleLoading(n,"",true,d);if(t===""){return}setTimeout(async()=>{if(!await e()){toggleLoading(n,t,true,d)}},i)}function createLoading(n){const e=type_loading();loadingCount.set(n,e);return e}function getLoadingKey(n){const e=srcBy$src.get(n);if(e===undefined){return n}if(loadingCount.has(e.id)){return e.id}const t=loadingCount.get(n);if(t===undefined){return n}loadingCount.delete(n);loadingCount.set(e.id,t);return e.id}function decLoading(n,e,t){if(e!==""){const i=t.get(e)-1;t.set(e,i);if(i<=0){toggleLoading(n,e,false,t)}}const i=t.get("")-1;t.set("",i);if(i>0){return}toggleLoading(n,"",false,t);for(const[o,g]of loadingCount){for(const[d,i]of g){if(i>0){continue}if(d===""){loadingCount.delete(o)}}}}function toggleLoading(n,e,t,i){if(is$hide(n)){console.warn(43243242);alert(1);return}const o=e===""?isFillingName:isFillingName+isFillingDiv+e;if(!t){n.removeAttribute(o,"");if(n.nodeName==="TEMPLATE"){n.content.firstChild.removeAttribute(o,"")}return}if(e===""){const g=i.get("");i.set("",g+1)}else{const g=i.get(e);if(g===undefined){i.set(e,1)}else{i.set(e,g+1)}}n.setAttribute(o,"");if(n.nodeName==="TEMPLATE"){n.content.firstChild.setAttribute(o,"")}}function type_loading(){return new Map([["",0]])}self.mw_loadingCount=loadingCount;self.mw_showLoading=showLoading;')],
