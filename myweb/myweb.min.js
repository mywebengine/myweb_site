...
["/cmd/exec.js", import('data:text/javascript;text,let p_target,eval2,q_eval2,kebabToCamelCase;import.meta.__imports__=Promise.all([self.__import__.get("/config.js"),self.__import__.get("/eval2.js"),self.__import__.get("/str.js")]).then(arr=>{let m;m=arr[0];p_target=m.p_target;m=arr[1];eval2=m.eval2;q_eval2=m.q_eval2;m=arr[2];kebabToCamelCase=m.kebabToCamelCase;});export default{render(r){return eval2(r,r.$src,true).then(e=>{exec_render(r,r.scope,e);return null})},q_render(n,a,o){return q_eval2(n,a,o).then(r=>{const t=a.length;for(let e=0;e<t;e++){if(!o.has(e)){exec_render(n,a[e].scope,r[e])}}return null})}};function exec_render(e,r,t){const n=e.reqCmd.args[0];if(n!==undefined&&n!==""){r[p_target][kebabToCamelCase(n)]=t}}')],
...
["/cmd/scope.js", import('data:text/javascript;text,let p_target,getErr,kebabToCamelCase;import.meta.__imports__=Promise.all([self.__import__.get("/config.js"),self.__import__.get("/err.js"),self.__import__.get("/str.js")]).then(arr=>{let m;m=arr[0];p_target=m.p_target;m=arr[1];getErr=m.getErr;m=arr[2];kebabToCamelCase=m.kebabToCamelCase;});export default{render(e){scope_render(e,e.scope);return null},q_render(o,t,n){return q_eval2(o,t,n).then(e=>{const r=t.length;for(let e=0;e<r;e++){if(!n.has(e)){scope_render(o,t[e].scope)}}return null})}};function scope_render(e,r){const o=e.reqCmd.args[0];if(o===undefined||o===""){throw getErr(new Error(">>>mw scope:render: Need set scope name"),e.$src,e)}r[p_target][kebabToCamelCase(o)]=r}')],
...
["/loading.js", import('data:text/javascript;text,let isFillingName,isFillingDiv,srcBy$src,is$hide;import.meta.__imports__=Promise.all([self.__import__.get("/config.js"),self.__import__.get("/descr.js"),self.__import__.get("/dom.js")]).then(arr=>{let m;m=arr[0];isFillingName=m.isFillingName;isFillingDiv=m.isFillingDiv;m=arr[1];srcBy$src=m.srcBy$src;m=arr[2];is$hide=m.is$hide;});export const loadingCount=new Map;export async function showLoading(t,e,n="",i){const o=getLoadingKey(t),g=loadingCount.get(o);if(await e()){if(g!==undefined){decLoading(t,n,g)}return}const d=g===undefined?createLoading(o):g;if(i===undefined||i===""){i=-1}if(i<0){toggleLoading(t,"",true,d);if(n!==""){toggleLoading(t,n,true,d)}return}toggleLoading(t,"",true,d);if(n===""){return}setTimeout(async()=>{if(is$hide(t)){return}if(!await e()){toggleLoading(t,n,true,d)}},i)}function createLoading(t){const e=type_loading();loadingCount.set(t,e);return e}function getLoadingKey(t){const e=srcBy$src.get(t);if(e===undefined){return t}if(loadingCount.has(e.id)){return e.id}const n=loadingCount.get(t);if(n===undefined){return t}loadingCount.delete(t);loadingCount.set(e.id,n);return e.id}function decLoading(t,e,n){if(e!==""){const i=n.get(e)-1;n.set(e,i);if(i<=0){toggleLoading(t,e,false,n)}}const i=n.get("")-1;n.set("",i);if(i>0){return}toggleLoading(t,"",false,n);for(const[o,g]of loadingCount){for(const[d,i]of g){if(i>0){continue}if(d===""){loadingCount.delete(o)}}}}function toggleLoading(t,e,n,i){const o=e===""?isFillingName:isFillingName+isFillingDiv+e;if(!n){t.removeAttribute(o,"");if(t.nodeName==="TEMPLATE"){t.content.firstChild.removeAttribute(o,"")}return}if(e===""){const g=i.get("");i.set("",g+1)}else{const g=i.get(e);if(g===undefined){i.set(e,1)}else{i.set(e,g+1)}}t.setAttribute(o,"");if(t.nodeName==="TEMPLATE"){t.content.firstChild.setAttribute(o,"")}}function type_loading(){return new Map([["",0]])}')],
["/evt.js", import('data:text/javascript;text,let defEventInit;import.meta.__imports__=Promise.all([self.__import__.get("/config.js")]).then(arr=>{let m;m=arr[0];defEventInit=m.defEventInit;});export function dispatchCustomEvent(t,n,e){const i={detail:e};for(const o in defEventInit){i[o]=defEventInit[o]}t.dispatchEvent(new CustomEvent(n,i))}')],
