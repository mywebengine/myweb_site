...
["/loc.js", import('data:text/javascript;text,let locVarName,oset,getProxy;self.__p.add(import.meta.__imports=Promise.all([self.__i.get("/config.js"),self.__i.get("/oset.js"),self.__i.get("/proxy.js")]).then(arr=>{let m;m=arr[0];locVarName=m.locVarName;m=arr[1];oset=m.oset;m=arr[2];getProxy=m.getProxy;}));export function setLoc(o){oset(self,locVarName,getLoc(o))}export function getLoc(o){o=new URL(o);const t=new Map;for(const[e,r]of o.searchParams){t.set(e,r)}return type_loc(o.origin,o.href,o.pathname,t,o.hash)}function type_loc(o,t,e,r,n){return{origin:o,href:t,pathname:e,query:r,hash:{path:n.substr(1)}}}')],
["/loading.js", import('data:text/javascript;text,let isFillingName,isFillingDiv,$srcById,srcBy$src,is$hide;self.__p.add(import.meta.__imports=Promise.all([self.__i.get("/config.js"),self.__i.get("/descr.js"),self.__i.get("/dom.js")]).then(arr=>{let m;m=arr[0];isFillingName=m.isFillingName;isFillingDiv=m.isFillingDiv;m=arr[1];$srcById=m.$srcById;srcBy$src=m.srcBy$src;m=arr[2];is$hide=m.is$hide;}));export const loadingCount=new Map;export async function showLoading(e,t,i="",n){const o=srcBy$src.get(e),s=o.id,r=loadingCount.get(s);if(await t()){if(r!==undefined){decLoading(e,i,r)}else{toggleLoading(e,i,false,r)}return}const g=r===undefined?createLoading(s):r;toggleLoading(e,"",true,g);if(i===""){return}if(n===undefined||n===""){n=-1}if(n<0){toggleLoading(e,i,true,g);return}setTimeout(async()=>{if(is$hide(e)){return}if(!await t()){toggleLoading(e,i,true,g)}},n)}function createLoading(e){const t=type_loading();loadingCount.set(e,t);return t}function decLoading(e,t,i){if(t!==""){const n=i.get(t)-1;i.set(t,n)}const n=i.get("")-1;i.set("",n);if(n>0){return}for(const[o,s]of loadingCount){if(s!==i){continue}loadingCount.delete(o);const r=$srcById.get(o);if(r===undefined){continue}for(const t of i.keys()){toggleLoading(r,t,false,i)}}}function toggleLoading(e,t,i,n){const o=t===""?isFillingName:isFillingName+isFillingDiv+t;if(!i){e.removeAttribute(o,"");if(srcBy$src.get(e).isHide){e.content.firstChild.removeAttribute(o,"")}return}if(t===""){const s=n.get("");n.set("",s+1)}else{const s=n.get(t);if(s===undefined){n.set(t,1)}else{n.set(t,s+1)}}e.setAttribute(o,"");if(srcBy$src.get(e).isHide){e.content.firstChild.setAttribute(o,"")}}function type_loading(){return new Map([["",0]])}')],
...
["/dom.js", import('data:text/javascript;text,let type_req,type_animation,type_cacheValue,type_cacheCurrent,mw_doc,mw_$src,p_topUrl,visibleScreenSize,defIdleCallbackOpt,incCmdName,onCmdName,textCmdName,descrIdName,asOneIdxName,idxName,hideName,removeEventName,defEventInit,reqCmd,$srcById,srcById,srcBy$src,descrById,getNewId,createSrc,type_asOneIdx,type_idx,getSrcId,get$els,getNextStr,getErr,loadingCount,varIdByVar,varById,srcIdsByVarId,varIdByVarIdByProp;self.__p.add(import.meta.__imports=Promise.all([self.__i.get("/render/render.js"),self.__i.get("/cache.js"),self.__i.get("/config.js"),self.__i.get("/descr.js"),self.__i.get("/err.js"),self.__i.get("/loading.js"),self.__i.get("/proxy.js")]).then(arr=>{let m;m=arr[0];type_req=m.type_req;type_animation=m.type_animation;m=arr[1];type_cacheValue=m.type_cacheValue;type_cacheCurrent=m.type_cacheCurrent;m=arr[2];mw_doc=m.mw_doc;mw_$src=m.mw_$src;p_topUrl=m.p_topUrl;visibleScreenSize=m.visibleScreenSize;defIdleCallbackOpt=m.defIdleCallbackOpt;incCmdName=m.incCmdName;onCmdName=m.onCmdName;textCmdName=m.textCmdName;descrIdName=m.descrIdName;asOneIdxName=m.asOneIdxName;idxName=m.idxName;hideName=m.hideName;removeEventName=m.removeEventName;defEventInit=m.defEventInit;reqCmd=m.reqCmd;m=arr[3];$srcById=m.$srcById;srcById=m.srcById;srcBy$src=m.srcBy$src;descrById=m.descrById;getNewId=m.getNewId;createSrc=m.createSrc;type_asOneIdx=m.type_asOneIdx;type_idx=m.type_idx;getSrcId=m.getSrcId;get$els=m.get$els;getNextStr=m.getNextStr;m=arr[4];getErr=m.getErr;m=arr[5];loadingCount=m.loadingCount;m=arr[6];varIdByVar=m.varIdByVar;varById=m.varById;srcIdsByVarId=m.srcIdsByVarId;varIdByVarIdByProp=m.varIdByVarIdByProp;}));export function preRender(e,t){const n=e.parentNode,r=[],i=new Map;do{if(e.firstChild!==null){e=e.firstChild;continue}if(e.nodeName==="TEMPLATE"&&e.getAttribute(hideName)!==null){r.push(e);e=e.content.firstChild;continue}e=_preRenderCreate(e,i,t);if(e.parentNode===n){break}if(e.nextSibling!==null){e=e.nextSibling;continue}do{e=e.parentNode;if(e.nodeType===11){e=r.pop()}e=_preRenderCreate(e,i,t);if(e.parentNode===n){e=null;break}if(e.nextSibling!==null){e=e.nextSibling;break}}while(true)}while(e!==null)}function _preRenderCreate(e,t,n){if(e.nodeType!==1){return e.nodeType===3?replaceTextBlocks(e):e}if(!n){createSrc(e);return e}const r=_preRenderGetSrc(e,t);if(!r.isCmd){return e}for(const i of r.descr.attr.keys()){const d=e.getAttribute(asOneIdxName+i),o=e.getAttribute(idxName+i);if(d!==null){const l=t.get(d);if(l===undefined){const s=getNewId();t.set(d,s);if(r.asOneIdx===null){r.asOneIdx=type_asOneIdx([[i,s]])}else{r.asOneIdx.set(i,s)}}else if(r.asOneIdx===null){r.asOneIdx=type_asOneIdx([[i,l]])}else{r.asOneIdx.set(i,l)}}if(o!==null){if(r.idx===null){r.idx=type_idx([[!isNaN(o)?Number(o):o]]);continue}r.idx.set(i,!isNaN(o)?Number(o):o)}}return e}function _preRenderGetSrc(e,t){const n=e.getAttribute(descrIdName);if(n===null){return createSrc(e)}const r=t.get(n);if(r!==undefined){return createSrc(e,r,null,null)}const i=createSrc(e,createDescr(e,0),null,null);i.descr.sId=i.id;t.set(n,i.descr);return i}function replaceTextBlocks(e){const t=e.textContent;if(t.indexOf("${")===-1||e.parentNode.nodeName==="SCRIPT"){return e}if(e.nextSibling===null&&e.previousSibling===null){e.parentNode.setAttribute(textCmdName,"`"+t+"`");return e}const n=mw_doc.createElement("span");n.setAttribute(textCmdName,"`"+t+"`");e.parentNode.replaceChild(n,e);createSrc(n);return n}export function joinText(t){t=t.firstChild;for(let e;t!==null;t=t.nextSibling){while(t.nodeType===3&&(e=t.nextSibling)!==null&&e.nodeType===3){t.textContent+=$e.removeChild(e).textContent}}}export function removeChild(e,t){e.parentNode.removeChild(e);e.dispatchEvent(new CustomEvent(removeEventName,defEventInit));const n=new Map;let r=e;const i=null,d=[];do{const o=srcBy$src.get(r);if(o!==undefined){clearTag(r,o,n);if(o.descr.isCustomHtml){continue}if(r.firstChild!==null){r=r.firstChild;continue}if(o.isHide&&o.isCmd&&r.content.firstChild.firstChild!==null){d.push(r);r=r.content.firstChild.firstChild;continue}}if(r.parentNode===i){break}if(r.nextSibling!==null){r=r.nextSibling;continue}do{r=r.parentNode;if(r.nodeType===11){r=d.pop()}if(r.parentNode===i){r=null;break}if(r.nextSibling!==null){r=r.nextSibling;break}}while(true)}while(r!==null);if(n.size===0){return}requestIdleCallback(()=>{clearVars(n)},defIdleCallbackOpt)}function clearTag(e,t,n){const r=t.id,i=t.descr,d=i.id;$srcById.delete(r);srcById.delete(r);srcBy$src.delete(e);if(!t.isCmd){i.srcIds.delete(r);if(i.srcIds.size===0){descrById.delete(d)}return}loadingCount.delete(r);i.srcIds.delete(r);if(i.sId===r){i.sId=i.srcIds.values().next().value}const o=n.get(d);if(o){o.add(r);return}n.set(d,new Set([r]))}function clearVars(e){const t=new Set,n=new Set,r=new Set;for(const[i,d]of e){const o=descrById.get(i);if(o===undefined){continue}for(const l of o.varIds){t.add(l)}for(const s of d){n.add(s)}}for(const l of t){const d=srcIdsByVarId.get(l);if(!d){continue}const c=varIdByVarIdByProp.get(l);for(const s of n){if(d.has(s)){d.delete(s)}if(c===undefined){continue}for(const[a,f]of c){const u=srcIdsByVarId.get(f);if(u!==undefined&&u.has(s)){d.delete(s);u.delete(s);if(u.size===0){r.add(f);srcIdsByVarId.delete(f);c.delete(a)}}}}if(d.size===0){r.add(l);srcIdsByVarId.delete(l);varIdByVar.delete(varById.get(l));varById.delete(l);if(c!==undefined){varIdByVarIdByProp.delete(l);if(c.size!==0){console.warn("должно быть пусто",l,c)}}}else if(c!==undefined&&c.size===0){varIdByVarIdByProp.delete(l)}}for(const[i,d]of e){const o=descrById.get(i);if(o===undefined){continue}for(const l of r){o.varIds.delete(l)}for(const s of d){o.srcIds.delete(s);if(o.sId===s){console.warn(22222222)}}if(o.srcIds.size===0){descrById.delete(i)}}}export function q_cloneNode(r,e,t,i){e=getSrcId(r.sync.local,e);const n=$srcById.get(e),d=srcById.get(e),o=getNextStr(d,r.str),l=o!==""?get$els(n,d.descr.get$elsByStr,o):[n],s=l.length,c=new Array(i),a=[];let f;for(let e=0,t;e<s;e++){const h=l[e];f=srcBy$src.get(h);if(f===undefined||!f.isCmd){continue}break}if(f===undefined){console.warn(">>>mw dom:q_cloneNode:",r,l,t,i);throw getErr(new Error(`>>>mw dom:q_cloneNode: среди элементов для клонирования нет элемента с командой, такого не должно быть`),r.$src,r)}const u=f.descr;for(const[N,I]of u.attr){if(N===r.str){break}const C=reqCmd.get(N);if(C.cmdName===onCmdName){a.push(type_cloneNodeOn(C.cmd,N,I))}}const p=a.length,m=loadingCount.get(f.id),x=new Set,g=new Array(i),y=u.asOnes.keys();for(let e=y.next();!e.done;e=y.next()){if(e.value!==r.str){continue}for(e=y.next();!e.done;e=y.next()){x.add(e.value)}}for(let e=0;e<i;e++){c[e]=type_q$i(new Array(s),e+t);g[e]=new Map}for(let e,t,n=0;n<s;n++){const b=new Array(i),_=l[n];q_cloneNodeCreate(_,_.nodeName!=="TEMPLATE"||_.getAttribute(hideName)===null,b,i,g,0,x);q_cloneNodeCreateChildren(_,b,i,g);for(e=0;e<i;e++){const w=c[e],h=w.$els[n]=b[e],v=srcBy$src.get(h);if(v===undefined){continue}setIdx(v,r.str,w.idx);if(p!==0){for(k=0;k<p;k++){const S=a[k];S.cmd.render(type_req(h,S.str,S.expr,r.scope,r.sync))}}if(m!==undefined){loadingCount.set(v.id,m)}}}return c}function q_cloneNodeCreateChildren(e,t,n,r){const i=new Array(n),d=e.parentNode,o=[];for(let e=0;e<n;e++){i[e]=[]}do{if(e.firstChild!==null){q_cloneNodeCreate(e=e.firstChild,true,t,n,r,1,null);continue}if(e.nodeName==="TEMPLATE"&&e.getAttribute(hideName)!==null){o.push(e);for(let e=0;e<n;e++){i[e].push(t[e])}q_cloneNodeCreate(e=e.content.firstChild,false,t,n,r,1,null);continue}if(e.parentNode===d){break}if(e.nextSibling!==null){q_cloneNodeCreate(e=e.nextSibling,true,t,n,r,2,null);continue}do{e=e.parentNode;if(e.nodeType!==11){for(let e=0;e<n;e++){t[e]=t[e].parentNode}}else{e=o.pop();for(let e=0;e<n;e++){t[e]=i[e].pop()}}if(e.parentNode===d){e=null;break}if(e.nextSibling!==null){q_cloneNodeCreate(e=e.nextSibling,true,t,n,r,2,null);break}}while(true)}while(e!==null)}function q_cloneNodeCreate(t,e,n,r,i,d,o){const l=srcBy$src.get(t);if(l===undefined){if(d===2){for(let e=0;e<r;e++){n[e]=n[e].parentNode.appendChild(t.cloneNode())}return}if(d===1){if(e){for(let e=0;e<r;e++){n[e]=n[e].appendChild(t.cloneNode())}return}for(let e=0;e<r;e++){n[e]=n[e].content.appendChild(t.cloneNode())}return}for(let e=0;e<r;e++){n[e]=t.cloneNode()}return}const s=l.descr,c=l.idx,a=l.save,f=l.asOneIdx;if(d===2){for(let e=0;e<r;e++){createSrc(n[e]=n[e].parentNode.appendChild(t.cloneNode()),s,type_asOneIdx(f),type_idx(l.idx)).save=a}if(f!==null){q_cloneNodeChangeAsOne(n,r,i,f)}return}if(d===1){if(e){for(let e=0;e<r;e++){createSrc(n[e]=n[e].appendChild(t.cloneNode()),s,type_asOneIdx(f),type_idx(l.idx)).save=a}if(f!==null){q_cloneNodeChangeAsOne(n,r,i,f)}return}for(let e=0;e<r;e++){createSrc(n[e]=n[e].content.appendChild(t.cloneNode()),s,type_asOneIdx(f),type_idx(l.idx)).save=a}if(f!==null){q_cloneNodeChangeAsOne(n,r,i,f)}return}for(let e=0;e<r;e++){createSrc(n[e]=t.cloneNode(),s,type_asOneIdx(f),type_idx(l.idx)).save=a}if(f!==null){q_cloneNodeChangeAsOne(n,r,i,o)}}function q_cloneNodeChangeAsOne(t,n,r,i){for(let e=0;e<n;e++){const d=t[e],o=srcBy$src.get(d);for(const l of i.keys()){const s=o.asOneIdx.get(l),c=r[e].get(s);if(c!==undefined){setAsOneIdx(o,l,c);continue}const a=getNewId();r[e].set(s,a);setAsOneIdx(o,l,a)}}}export function type_q$i(e,t){return{$els:e,idx:t}}export function type_cloneNodeOn(e,t,n){return{cmd:e,str:t,expr:n}}export function setAttribute(e,t,n){setAttributeValue(e,t,n);e.setAttribute(t,n)}export function setAttributeValue(e,t,n){switch(t){case"value":if(e===document.activeElement&&typeof e.setSelectionRange==="function"){const r=e.selectionStart;e.value=n;if(e.type==="text"||e.type==="search"){e.setSelectionRange(r,r)}}else{e.value=n}break;case"checked":e.checked=!!n;break}}export function removeAttribute(e,t){switch(t){case"value":e.value="";break;case"checked":e.checked=false;break}e.removeAttribute(t)}export function show(e,t){const n=srcBy$src.get(t);if(n!==undefined?!n.isHide:t.nodeName!=="TEMPLATE"||t.getAttribute(hideName)===null){return}e.sync.animations.add(type_animation(()=>_show(e,t,n),e.sync.local,0))}function _show(e,t,n){const r=t.content.firstChild;if(!r||r.nextSibling!==null){throw getErr(new Error(">>>mw show:01: Template element invalid structure on show function. <template>.content.childNodes.length must be only one element."),t)}if(n!==undefined){moveProps(t,n,r,false)}if(e.$src===t){e.$src=r}t.parentNode.replaceChild(r,t)}export function hide(e,t){const n=srcBy$src.get(t);if(n!==undefined){if(!n.isHide){e.sync.animations.add(type_animation(()=>_hide(t,n),e.sync.local,n.id))}return}if(t.nodeType===1?t.nodeName==="TEMPLATE"&&t.getAttribute(hideName)!==null:t.nodeType!==8){e.sync.animations.add(type_animation(()=>_hide(t,n),e.sync.local,0))}}function _hide(e,t){let n=e;const r=mw_doc.createElement("template"),i=n.parentNode,d=[];r.setAttribute(hideName,"");do{const o=srcBy$src.get(n);if(o!==undefined){const l=o.cache;if(l!==null){l.value=type_cacheValue();l.current=type_cacheCurrent()}if(n.firstChild!==null){n=n.firstChild;continue}if(o.isHide&&n.content.firstChild.firstChild!==null){d.push(n);n=n.content.firstChild.firstChild;continue}}if(n.parentNode===i){break}if(n.nextSibling!==null){n=n.nextSibling;continue}do{n=n.parentNode;if(n.nodeType===11){n=d.pop()}if(n.parentNode===i){n=null;break}if(n.nextSibling!==null){n=n.nextSibling;break}}while(true)}while(n!==null);if(t!==undefined){moveProps(e,t,r,true)}e.parentNode.replaceChild(r,e);r.content.appendChild(e)}function moveProps(e,t,n,r){t.isHide=r;$srcById.set(t.id,n);srcById.set(t.id,t);srcBy$src.set(n,t);n[p_topUrl]=e[p_topUrl];if(self.mw_debugLevel===0){return}if(!r){return}const i=e.attributes,d=i.length;for(let e=0;e<d;e++){const o=i[e];setAttribute(n,o.name,o.value)}}export function is$hide(e){do{if(e===mw_$src){return false}e=e.parentNode}while(e!==null);return true}export function isAnimationVisible(e){return e.viewedSrcId===0?true:is$visible($srcById.get(getSrcId(e.local,e.viewedSrcId)))}const $scroll=document.scrollingElement;export function is$visible(e){while(e.nodeType!==1){e=e.nextSibling;if(e===null){return true}}const t=1-visibleScreenSize,n=$scroll.clientWidth*t,r=$scroll.clientWidth+n*-1,i=$scroll.clientHeight*t,d=$scroll.clientHeight+i*-1,o=e.getBoundingClientRect();return!(o.top>d||o.top+o.height<i||(o.left>r||o.left+o.width<n))}export function setAsOneIdx(e,t,n){if(e.asOneIdx===null){e.asOneIdx=type_asOneIdx()}e.asOneIdx.set(t,n);if(self.mw_debugLevel===0){return}const r=$srcById.get(e.id),i=asOneIdxName+t;r.setAttribute(i,n);if(e.isHide&&r.content.firstChild!==null){r.content.firstChild.setAttribute(i,n)}}export function getIdx(e,t){if(e.idx!==null){return e.idx.get(t)}}export function setIdx(e,t,n){if(e.idx===null){e.idx=type_idx()}e.idx.set(t,n);if(self.mw_debugLevel===0){return}const r=$srcById.get(e.id),i=idxName+t;r.setAttribute(i,n);if(e.isHide){r.content.firstChild.setAttribute(i,n)}}export function getTopUrl(t,e){if(e!==""){const n=getAttrTopUrl(t,e);if(n!==""){return n}}for(let e=$srcById.get(t.id).parentNode;e!==mw_$src;e=e.parentNode){const n=getAttrTopUrl(srcBy$src.get(e));if(n!==""){return n}if(e[p_topUrl]!==undefined){return e[p_topUrl]}}return""}function getAttrTopUrl(e,t){if(!e.isCmd){return""}const n=e.descr.attr.keys();let r="";if(t!==""){for(const i of n){if(i===t){break}if(reqCmd.get(i).cmdName===incCmdName){r=getIdx(e,i)}}return r}for(const i of n){if(reqCmd.get(i).cmdName===incCmdName){r=getIdx(e,i)}}return r}')],
...
["/evt.js", import('data:text/javascript;text,let defEventInit;self.__p.add(import.meta.__imports=Promise.all([self.__i.get("/config.js")]).then(arr=>{let m;m=arr[0];defEventInit=m.defEventInit;}));export function dispatchCustomEvent(t,n,e){const i={detail:e};for(const o in defEventInit){i[o]=defEventInit[o]}t.dispatchEvent(new CustomEvent(n,i))}')],
["/descr.js", import('data:text/javascript;text,let setReqCmd,type_cache,reqCmd,incCmdName,hideName,setAsOneIdx,getIdx,setIdx,getProxy;self.__p.add(import.meta.__imports=Promise.all([self.__i.get("/render/render.js"),self.__i.get("/cache.js"),self.__i.get("/config.js"),self.__i.get("/dom.js"),self.__i.get("/proxy.js")]).then(arr=>{let m;m=arr[0];setReqCmd=m.setReqCmd;m=arr[1];type_cache=m.type_cache;m=arr[2];reqCmd=m.reqCmd;incCmdName=m.incCmdName;hideName=m.hideName;m=arr[3];setAsOneIdx=m.setAsOneIdx;getIdx=m.getIdx;setIdx=m.setIdx;m=arr[4];getProxy=m.getProxy;}));export const $srcById=new Map;export const srcById=new Map;export const srcBy$src=new WeakMap;export const descrById=self.mw_descrById||new Map;export let idCurVal=self.mw_idCurVal||0;self.mw_$srcById=$srcById;self.mw_srcById=srcById;self.mw_srcBy$src=srcBy$src;export function getNewId(){return++idCurVal}export function createSrc(e,t,r,n){const s=getNewId(),c=e.getAttribute(hideName)!==null;if(t===undefined){t=createDescr(e,s);const o=t.attr!==null?type_src(s,t,true,c,null,null,type_cache()):type_src(s,t,false,c,null,null,null);$srcById.set(s,e);srcById.set(s,o);srcBy$src.set(e,o);return o}t.srcIds.add(s);const o=t.attr!==null?type_src(s,t,true,c,r,n,type_cache()):type_src(s,t,false,c,null,null,null);$srcById.set(s,e);srcById.set(s,o);srcBy$src.set(e,o);if(!o.isCmd){return o}return o}export function createDescr(e,t){const r=getNewId(),n=createAttr(e);if(n.size===0){const s=type_descr(r,t,null,null);descrById.set(r,s);return s}const s=type_descr(r,t,n,new Set);let c=0;for(const[o,u]of n){const d=reqCmd.get(o);if(d.cmd.get$els){if(s.get$elsByStr===null){s.get$elsByStr=type_get$elsByStr()}s.get$elsByStr.set(o,type_get$elsByStrI(u,c))}c++;if(d.cmd.isCustomHtml&&s.isCustomHtml===false){s.isCustomHtml=true}if(d.cmd.isAsOne){if(s.asOnes===null){s.asOnes=new Set}s.asOnes.add(o)}}descrById.set(r,s);return s}function type_src(e,t,r,n,s,c,o){return{id:e,descr:t,isCmd:r,isHide:n,asOneIdx:s,idx:c,save:null,cache:o,scopeCache:getProxy({}),isMounted:false}}export function type_asOneIdx(e){return new Map(e)}export function type_idx(e){return new Map(e)}export function type_save(e){if(e===undefined){return new Map}const t=new Map;for(const[r,n]of e){t.set(r,type_saveI(n))}return t}export function type_saveI(e){return new Map(e)}function type_descr(e,t,r,n){return{id:e,sId:t,attr:r,varIds:n,srcIds:new Set([t]),isCustomHtml:false,asOnes:null,get$elsByStr:null}}function type_get$elsByStr(){return new Map}function type_get$elsByStrI(e,t){return{expr:e,pos:t}}export function createAttr(e){const t=new Map,r=e.attributes,n=r.length;for(let e=0;e<n;e++){const s=r[e];if(setReqCmd(s.name)){t.set(s.name,s.value)}}return t}export function getSrcId(t,r){if(srcById.has(r)){return r}for(let e=t.get(r);e!==undefined&&e.newSrcId!==0;e=t.get(r)){r=e.newSrcId}return r}export function get$els(e,t,r){const n=srcBy$src.get(e).descr.attr.keys();let s=n.next();if(r!==""){for(;!s.done;s=n.next()){if(s.value===r){break}}}while(!s.done){const c=s.value,o=t.get(c);if(o!==undefined){return reqCmd.get(c).cmd.get$els(e,c,o.expr,o.pos)}s=n.next()}return[e]}export function get$first(e,t,r){const n=srcBy$src.get(e).descr.attr.keys();let s=n.next();if(r!==""){for(;!s.done;s=n.next()){if(s.value===r){break}}}while(!s.done){const c=s.value,o=t.get(r);if(o!==undefined){return reqCmd.get(r).cmd.get$first(e,r,o.expr,o.pos)}s=n.next()}return e}export function getNextStr(e,t){const r=e.descr.attr.keys();for(let e=r.next();!e.done;e=r.next()){if(e.value!==t){continue}e=r.next();if(e.done){return""}return e.value}return""}export function getAttrAfter(e,t){const r=new Map,n=getAttrItAfter(e.entries(),t,true);for(let e=n.next();!e.done;e=n.next()){r.set(e.value[0],e.value[1])}return r}export function getAttrItAfter(t,r,e){if(r===""){return t}if(e){for(let e=t.next();!e.done;e=t.next()){if(e.value[0]===r){return t}}return t}for(let e=t.next();!e.done;e=t.next()){if(e.value===r){return t}}return t}')],
